...
void hydra_restore_write(int print_msg) {
  ...
  time_t total_runtime = (*(times.time0) + time(NULL) - *(times.starttime));
  fck = fwrite(&total_runtime, sizeof(total_runtime), 1, f);
  fprintf(f, "%s\n", PROGRAM);
...
void hydra_restore_read() {
  ...
  //Load old run-time
  fck = (int) fread(times.time0, sizeof(*(times.time0)), 1, f);
...
int main(int argc, char *argv[]) {
  ...
  time_t starttime, time0 = 0, elapsed_status, elapsed_restore, status_print = 59, tmp_time;
...
//      if (debug) printf("[DEBUG] head_no[%d] active %d\n", head_no, hydra_heads[head_no]->active);
    ...
    if (time(NULL) - elapsed_status > status_print) {
      ...
      time_t total_elapsed = elapsed_status - starttime + time0; //Total program running time
      ...
      if (((hydra_brains.todo_all - hydra_brains.sent) / tmp_time) < 150)
        status_print = 59;*/
      k = 0;  //number of active threads
      //count the active threads
      for (j = 0; j < hydra_options.max_use; j++)
        if (hydra_heads[j]->active >= 0)
          k++;
      printf("[STATUS] %.2f tries/min, %lu tries in %02lu:%02luh, %lu todo in %02lu:%02luh, %d/%d active\n", (1.0 * hydra_brains.sent) / (((total_elapsed) * 1.0) / 60),      // tries/min 
             hydra_brains.sent, // tries 
             (long unsigned int) ((total_elapsed) / 3600), // hours 
             (long unsigned int) (((total_elapsed) % 3600) / 60),  // minutes 
             hydra_brains.todo_all - hydra_brains.sent < 0 ? 1 : hydra_brains.todo_all - hydra_brains.sent,     // left todo 
             (long unsigned int) (((double) hydra_brains.todo_all - hydra_brains.sent) / ((double) hydra_brains.sent / (total_elapsed))
             ) / 3600,          // hours 
             (((long unsigned int) (((double) hydra_brains.todo_all - hydra_brains.sent) / ((double) hydra_brains.sent / (total_elapsed))
               ) % 3600) / 60) + 1,     // min 
             k,                        //number of threads active
             hydra_options.max_use);  //maximum of threads
    }
